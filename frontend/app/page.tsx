...
interface ScanPair {
  ...
  matchStatus?: 'idle' | 'loading' | 'matched' | 'not-matched' | 'ambiguous' | 'error';
...
export default function Home() {
  ...
  const checkApproval = async (noBapp: string): Promise<{ status: ScanPair['matchStatus'], data: ApprovalData[], selected?: ApprovalData }> => {
    if (!noBapp) return { status: 'idle', data: [] };

    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_APPROVAL_API_URL}/api/is-approved?no_bapp=${encodeURIComponent(noBapp)}`);
  ...
  const processOcr = async (scannedData: ScanPair[]) => {
    ...
    for (let i = 0; i < updatedResults.length; i++) {
      ...
      if (updatedResults[i].docName && updatedResults[i].ocrStatus === 'success') {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 flex items-center justify-center p-4">
      ...
      <div className={`transition-all duration-500 ease-in-out ${viewMode === 'start' ? 'w-full max-w-xl' : 'w-full max-w-[95%]'
          ...
          {viewMode === 'start' && (
            <div className="space-y-6 animate-in fade-in zoom-in duration-300">
              <div className="text-center mb-8">
                ...
                <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">Siap untuk Memindai?</h2>
                <p className="text-gray-500 dark:text-gray-400">Pastikan scanner menyala dan dokumen sudah siap.</p>
              </div>
          ...
          {viewMode === 'results' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex justify-between items-center bg-gray-50 dark:bg-slate-800/50 p-4 rounded-lg border border-gray-200 dark:border-slate-700">
                <div>
                  <h2 className="text-lg font-bold text-gray-800 dark:text-white">Hasil Scan</h2>
                  <p className="text-sm text-gray-500">{scanResults.length} Dokumen ditemukan</p>
                </div>
                <div className="flex gap-2">
                  ...
                  <div className="flex gap-2">
                    <div className="px-3 py-1.5 rounded-lg bg-green-100 text-green-700 border border-green-200 text-xs font-bold flex items-center gap-1" title="Data Sesuai BAPP">
                      <span>✅</span> {scanResults.filter(r => r.matchStatus === 'matched' && r.selectedApproval?.hasil_cek === 'sesuai').length} Sesuai
                    </div>
                    <div className="px-3 py-1.5 rounded-lg bg-red-100 text-red-700 border border-red-200 text-xs font-bold flex items-center gap-1" title="Data Tidak Sesuai / Tidak Ditemukan">
                      <span>❌</span> {scanResults.filter(r => (r.matchStatus === 'matched' && r.selectedApproval?.hasil_cek !== 'sesuai') || r.matchStatus === 'not-matched').length} Tidak
                    </div>
                    <div className="px-3 py-1.5 rounded-lg bg-amber-100 text-amber-700 border border-amber-200 text-xs font-bold flex items-center gap-1" title="Butuh Verifikasi Manual">
                      <span>⚠️</span> {scanResults.filter(r => r.matchStatus === 'ambiguous').length} Duplikat
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  >
                          ...
                          <div className="flex gap-2 items-center mt-1">
                            {pair.ocrStatus === 'success' && (
                              <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-green-600 dark:text-green-400">
                                <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" /> OCR Terverifikasi
                              </span>
                            )}
                            {pair.ocrStatus === 'processing' && (
                              <span className="text-[10px] font-bold uppercase text-blue-500 animate-pulse">Memproses OCR...</span>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mb-5">
                      ...
                      {pair.matchStatus === 'matched' && (
                        <div className={`flex items-center justify-between p-3 rounded-xl font-bold text-sm border ${pair.selectedApproval?.hasil_cek === 'sesuai'
                          ...
                          <div className="flex items-center gap-2">
                            {pair.selectedApproval?.hasil_cek === 'sesuai' ? '✅ Status: Sesuai' : '⚠️ Status: Tidak Sesuai'}
                          </div>
                          <span className="text-[10px] opacity-70 uppercase tracking-widest">Verifikasi Selesai</span>
                        </div>
                      )}
                      ...
                      {pair.matchStatus === 'not-matched' && (
                        <div className="flex flex-col sm:flex-row justify-between items-center bg-red-50 dark:bg-red-900/20 p-3 rounded-xl border border-red-200 dark:border-red-800 gap-3">
                          <span className="text-sm font-semibold text-red-700 dark:text-red-400 italic font-mono">Data BAPP Tidak Ditemukan</span>
                          <button onClick={() => handleManualCheck(index)} className="w-full sm:w-auto px-4 py-1.5 bg-white dark:bg-slate-800 text-red-600 rounded-lg border border-red-200 shadow-sm hover:bg-red-50 transition-colors font-bold text-xs">
                            Cari Manual
                          </button>
                        </div>
                      )}
                      ...
                      <div className="mb-6 animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="bg-amber-100 dark:bg-amber-900/30 border-l-4 border-amber-500 p-4 rounded-r-2xl">
                          <p className="text-sm font-bold text-amber-800 dark:text-amber-200 mb-3 flex items-center gap-2 uppercase tracking-tight">
                            ...
                            ⚠️ Ditemukan {pair.approvalData?.length || 0} data duplikat. Pilih yang benar:
                          </p>
                              ...
                              >
                                <div className="text-xs space-y-1">
                                  <div className="flex gap-2">
                                    ...
                                    <span className="font-bold text-slate-700 dark:text-slate-200">{choice.npsn}</span>
                                  </div>
                                  <div className="flex gap-2 text-[10px]">
                                    <span className="text-slate-400 font-medium tracking-widest uppercase">Serial:</span>
                                    <span className="font-mono text-slate-600 dark:text-slate-300">{choice.sn_bapp}</span>
                                  </div>
                                </div>
                                <div className="flex items-center gap-3">
                                  <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase ${choice.hasil_cek === 'sesuai' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                    {choice.hasil_cek}
                                  </span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {[
                        ...
                        { label: 'Halaman Belakang', src: pair.front, key: 'front' }
                      ].map((img) => (
                        <div key={img.key} className="space-y-3">
                          <div className="flex justify-between items-center px-1">
                            <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">{img.label}</span>
                            {!img.src && <span className="text-[10px] text-red-400 italic">File Hilang</span>}
                          </div>
